...
# ╔═╡ df42905c-073f-11ef-3b33-7f046fbd8404
begin
	using Distributions: Normal, Uniform, MixtureModel, Chisq, pdf
	using Plots
...
md"# General problem definition"

# ╔═╡ 71e489fa-043c-41b9-b194-02bf4bb55b79
struct Params
        dt::Float64  # time step
        maxT::Float64  # Max time
        ts::Vector{Float64}
        ks::Vector{Int} # k=0 corresponds to x0
        Cd::Float64
        ρ::Float64

        # Constructor
        function Params(dt::Float64, maxT::Float64, Cd::Float64, ρ::Float64)
            ts = collect(0:dt:maxT)
            ks = collect(1:length(ts))
            new(dt, maxT, ts, ks, Cd, ρ)
        end
    end

# ╔═╡ 4ce87a14-64ac-46b3-89e7-6f2ff235e052
params = Params(0.5, 100.0, 0.1, 1.0)

# ╔═╡ 5e560828-c14a-4e6d-85b3-7b1bdefc778a
md"""
...
# Basic EOMs
function generalFlyingEOM(dx_vec::Vector{Float64}, x_vec::Vector{Float64}, p_vec::Vector{Float64}, t)
    # x_vec: [x, y, α, v, wx, wz]
...
function dynamicsUpdate(xk, noise, dynamics::Function, params)
	"""
	This updates the dynamics 1 step into the future and samples the noise
	inputs:
		x: the state vector[x, y, α, v, wx, wz]
		noise: the noise vector (everything has to be samplable by rand ie if you want a number just use [numb])
		dynamics: a function in the form (dx_vec, x_vec, [noise, Cd, rho], t) 
		params: a struct with {dt:time step, Cd: coeff of drag, \rho:density of air}

	Note: 
		Just make sure that the noise input matches what the dynamics needs... with cd and rho at the end
	"""

	noise_samp = rand.(noise)

	if size(noise_samp)[1] != 6
		throw("Noise size wrong")
	end

	# Create inputs for dynamics
	inputstuf = noise_samp
	push!(inputstuf, params.Cd)
	push!(inputstuf, params.ρ)
...
function genFixedWingTrajectory(x0::Vector{Float64}, params::Params)
	"""
	This generates a trajectory for a fixed wing plane. It makes reasonable controls.
	...
	for k in params.ks[2:end]
		push!(xks, dynamicsUpdate(xks[end], noise, generalFlyingEOM, params))
	end

	return stack(xks, dims=1) 
...
function genMultirotorTrajectory(x0::Vector{Float64}, params::Params)
	"""
	This generates a trajectory for a multirotor. It makes reasonable controls.
	Main difference from FixedWingGeneration is that it can take sharp turns sometimes.
	Returns a trajectory starting at x0 which is k=0 in matrix form. (k is first axis)
	Holds noise constant throught a time step
	"""
	...
	# Loop through all the time
	for k in params.ks[2:end] # The 2:end is becuase we allready have x0
		push!(xks, dynamicsUpdate(xks[end], noise, generalFlyingEOM, params))
	end
	return stack(xks, dims=1) 
...
begin
	# plot results states
	stateList = ["x", "z", "α [rad]", "V", "wx", "wz"] #Definded before
...
# Check size
@printf("The size of the xs are: %f and %f, and they should be %f", size(fixedWingTrajectory)[1], size(multirotorTrajectory)[1], length(params.ks))

# ╔═╡ ae7e355c-8f9c-416f-b592-3179c1961197
md"## Measurements"

# ╔═╡ 883be381-508e-47f2-92f4-b6c7213be02a
function radarMeasure(x::Vector{Float64}, p::Vector{Float64}, noise)
    """
    Simulates a radar measuring a object flying through space. Only returns the measured center of mass of the object. 

    Inputs:
        x: the state of the aircraft. [x, y, α, v, wx, wz]
	...
	# Unpacking
    dp = x[1:2] - p # get the delta position [x,y,z]
    α = x[3]
    v = x[4]
	
	el = atan(dp[2], dp[1]) + noise_samp[1]
    r = norm(dp) + noise_samp[2]
	# Velocity in global frame
    v_vector = [cos(α), sin(α)].*v
	# Get the component of the velocity pointing in the direction of the radar (same direction as dp) 
    rd = dot(dp, v_vector)/norm(dp) + noise_samp[3]
...
# ╔═╡ 325bd7a0-5e07-406f-bdcb-e4d92bbda1e5
function radarMeasure(xs::Matrix{Float64}, p::Vector{Float64}, noise)
    """
    Simulates a radar measuring a object flying through space. Only returns the measured center of mass of the object. 
	Just runs the function of same name for 1 x, but on all the x

    Inputs:
        xs: the matrix of states of the aircraft through discrete time. [x, y, α, v], dim 1 of matrix is k
		p: Radar position [x,z] in meters from global frame
		noise: noise vector for the measurements [el, r, rd]. Should be samplable by rand()
    Outputs:
    ...
    for i in 1:size(xs)[1]
        y = radarMeasure(xs[i,:], p, noise)
        push!(ys, y)
    end

	# Stack them into a matrix for convinience
	ys = stack(ys, dims=1)

    return ys::Matrix{Float64}
end

# ╔═╡ 4b89a88e-ea04-4f01-af52-a137963111e4
function y2p(ys::Matrix{Float64}, radar_p)
	"""
	Helper function to convert ys to x,z positions
	inputs:
		y: measurements
		p: Radar position [x,z] in meters from global frame
	"""
    ...
    for i in 1:size(ys)[1]
		# Unpack measurement for readability
		el = ys[i,1]
	    r = ys[i,2]
	    rd = ys[i,3]
		...
		# Convert to position
		dP = [cos(el), sin(el)].*r
    	p_measure = dP + radar_p
        push!(ps, p_measure)
    end
    return stack(ps, dims=1)::Matrix{Float64}
end
...
# ╔═╡ ff85c64a-b85c-4558-9068-41ddb869e397
# Check size
@printf("The size of the ys are: %f and %f, and they should be %f", size(fixedWingTrajectoryMeasurements)[1], size(multirotorTrajectoryMeasurements)[1], length(params.ks))

# ╔═╡ cec1b9bf-76a9-4e6d-b45b-ca4c35c0c39e
begin
		# Convert measurements to positions
		fixedWingTrajectoryMeasurementsPositions = y2p(fixedWingTrajectoryMeasurements, radar_p)
		multirotorTrajectoryMeasurementsPositions = y2p(multirotorTrajectoryMeasurements, radar_p)
...
begin
	function radarMeasureLikelihood(y::Vector{Float64}, x::Vector{Float64}, radar_p, radar_noise)
	    """
	    ...
	    y_pred = radarMeasure(x, radar_p, [[0.0],[0.0],[0.0]])
	
	    Λ = Vector{Float64}(undef, 3)
	    Λ[1] = pdf(radar_noise[1], y[1] - y_pred[1]) # el
	    Λ[2] = pdf(radar_noise[2], y[2] - y_pred[2]) # r
	    Λ[3] = pdf(radar_noise[3], y[3] - y_pred[3]) # rdot
	    return prod(Λ)::Float64
	
	end
	function radarMeasureLikelihood(y::Vector{Float64}, xs::Matrix{Float64}, radar_p, radar_noise)
		Λs = Vector{Float64}()
		for i in 1:size(xs)[1]
			push!(Λs, radarMeasureLikelihood(y, xs[i,:], radar_p, radar_noise))
		end
		return Λs::Vector{Float64}
	end
end
...
begin
	...
	#stateList = ["x", "z", "α", "V", "wx", "wz"] #defined before
	...
	plot(pfstateGenPlots[1], pfstateGenPlots[2], pfstateGenPlots[3], pfstateGenPlots[4], pfstateGenPlots[5], pfstateGenPlots[6], layout=(6,1), size=(800,1200), legend = true)
end
...
# ╔═╡ 30cff30b-6358-4713-bec4-77e78e9de313
md"**Set the number of particles for the PF**"

# ╔═╡ 250bebdc-ece9-4245-9153-bd659db34f18
# Particle filiter setup
@bind nP NumberField(100:10000, default=1000)
